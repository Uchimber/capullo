...
export default function AdminBookingsClient() {
  ...
  const statusBadge = (status: string) => {
    const map: Record<string, { bg: string; text: string; label: string }> = {
      PAID: { bg: 'bg-emerald-50 border-emerald-100', text: 'text-emerald-600', label: 'Төлөгдсөн' },
      PENDING: { bg: 'bg-peach/30 border-mauve/20', text: 'text-mauve', label: 'Хүлээгдэж буй' },
      CANCELLED: { bg: 'bg-rose-50 border-rose-100', text: 'text-rose-500', label: 'Цуцлагдсан' },
      BLOCKED: { bg: 'bg-foreground border-foreground', text: 'text-white', label: 'Завгүй' },
    }
    const s = map[status] || map.PENDING
    return (
      <span className={`px-4 py-2 rounded-full text-[9px] font-bold uppercase tracking-widest border shadow-sm ${s.bg} ${s.text}`}>
        {s.label}
      </span>
    )
  }
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-blush/10 border-b border-rose-soft/30">
                <th className="px-8 py-6 text-[10px] font-bold uppercase tracking-widest text-dusty">Үйлчлүүлэгч</th>
                <th className="px-8 py-6 text-[10px] font-bold uppercase tracking-widest text-dusty">Үйлчилгээ</th>
                <th className="px-8 py-6 text-[10px] font-bold uppercase tracking-widest text-dusty">Цаг</th>
                <th className="px-8 py-6 text-[10px] font-bold uppercase tracking-widest text-dusty">Төлөв</th>
                <th className="px-8 py-6 text-[10px] font-bold uppercase tracking-widest text-dusty text-right">Үйлдэл</th>
              </tr>
            </thead>
        ...
        {totalPages > 1 && (
          <div className="px-8 py-5 border-t border-rose-soft/30 bg-blush/5 flex items-center justify-between">
            <p className="text-xs font-bold text-dusty">
              Хуудас <span className="text-foreground">{page}</span> / {totalPages}
            </p>
