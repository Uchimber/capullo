...
export async function POST(req: Request) {
  try {
    ...
    // Capture various possible status and ID fields
    const status = (body.status || body.payment_status || '').toString().toUpperCase();
    const transactionId = body.transactionId || body.orderId || body.id || body.bookingId;
    const invoiceId = body.invoiceId || body.id;

    console.log(`Bonum Webhook processing: status=${status}, transactionId=${transactionId}, invoiceId=${invoiceId}`);
